...
class BiEncoderTrainer(object):
    ...
    def get_data_iterator(
        ...
        if is_train_set:
            [ds.load_data(i) for i, ds in enumerate(datasets_list)]
        else:
            [ds.load_data(-1) for i, ds in enumerate(datasets_list)]
    ...
    def run_train(self):
        ...
        if cfg.train.warmup_rate:
            warmup_steps = int(cfg.train.warmup_rate * total_updates) 
        else:
            warmup_steps = cfg.train.warmup_steps if cfg.train.warmup_steps else 0
    ...
    def validate_and_save(self, epoch: int, iteration: int, scheduler):
        ...
        if save_cp:
            cp_name = self._save_checkpoint(scheduler, epoch, iteration, "last")
            ...
            if validation_loss < (self.best_validation_result or validation_loss + 1):
                ...
                self.best_cp_name = self._save_checkpoint(scheduler, epoch, iteration, "best")                
    ...
    def validate_nll(self) -> float:
        ...
        for i, samples_batch in enumerate(data_iterator.iterate_ds_data()):
            ...
            biencoder_input = BiEncoder.create_biencoder_input2(
                ...
                samples_batch,
                self.tensorizer,
                self.cfg.train.valid_insert_title,
                num_hard_negatives,
                num_other_negatives,
                shuffle=False,
            )
    ...
    def _train_epoch(
        ...
        for i, samples_batch in enumerate(train_data_iterator.iterate_ds_data(epoch=epoch)):
            ...
            biencoder_batch = BiEncoder.create_biencoder_input2(
                ...
                samples_batch,
                self.tensorizer,
                self.cfg.train.train_insert_title,
                num_hard_negatives,
                num_other_negatives,
                shuffle=True,
    ...
    def _save_checkpoint(self, scheduler, epoch: int, offset: int, name: str) -> str:
        cfg = self.cfg
        model_to_save = get_model_obj(self.biencoder)
        output_dir = cfg.output_dir if cfg.output_dir else "./"
        cp = os.path.join(output_dir, cfg.checkpoint_file_name + "." + name)
