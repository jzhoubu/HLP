...
class BiEncoder(nn.Module):
    ...
    def create_biencoder_input2(
        ...
        for sample in samples:
            ...
            sample_ctxs_tensors = [
                tensorizer.text_to_tensor(ctx.text, title=ctx.title if (insert_title and ctx.title) else None, max_len = cfg.train.p_max_length)
                for ctx in all_ctxs
            ]
            ...
            #    else:
            #        question_tensors.append(tensorizer.text_to_tensor(" ".join([query_token, question]), max_len = cfg.train.q_max_length))
            #else:
            question_tensors.append(tensorizer.text_to_tensor(question, max_len = cfg.train.q_max_length))
        ...
        src_ids = [s.src_id for s in samples]
        src_ids_tensor = torch.LongTensor(src_ids)

        #print(f"### q_ids ### : {questions_tensor[0]}")
        #print(f"### ctxs_ids ### : {ctxs_tensor[:2]}")
