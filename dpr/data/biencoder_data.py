...
from dpr.utils.data_utils import read_data_from_json_files, read_data_from_jsonl_files, Tensorizer
...
class JsonQADataset(Dataset):
    def __init__(
        ...
        self.type2int = {} # to map sample type into integer
        logger.info("Data files: %s", self.data_files)

    def load_data(self, src_id=None):
...
class JsonlTokenQADataset(JsonQADataset):
    def load_data(self, src_id=None):
        self.data_files = get_dpr_files(self.file)
        data = []
        for data_file in self.data_files:
            with open(data_file, "r", encoding="utf-8") as f:
                logger.info("Reading file %s" % data_file)
                for l in tqdm(f):
                    sample = json.loads(l)
                    sample["question"] = np.array(sample["question"])    # reduce memory allocation          
                    for ctx in sample["positive_ctxs"] + sample.get("negative_ctxs", []) + sample.get("hard_negative_ctxs", []):
                        ctx["text"] = np.array(ctx["text"])
                        ctx["title"] = None if "title" not in ctx else np.array(ctx["title"])
                    data.append(sample)
    ...
    def __getitem__(self, index) -> BiEncoderSample:
        json_sample = self.data[index]
        r = BiEncoderSample()
        r.query = json_sample["question"]  # need process anchor position
        r.src_id = json_sample["src_id"]
        positive_ctxs = json_sample["positive_ctxs"]
        negative_ctxs = json_sample["negative_ctxs"] if "negative_ctxs" in json_sample else []
        hard_negative_ctxs = json_sample["hard_negative_ctxs"] if "hard_negative_ctxs" in json_sample else []
